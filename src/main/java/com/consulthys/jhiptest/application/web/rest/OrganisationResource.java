...
public class OrganisationResource {
    ...
    @Timed
    public ResponseEntity<List<OrganisationDTO>> getAllOrganisations(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("membership-is-null".equals(filter)) {
            log.debug("REST request to get all Organisations where membership is null");
            return new ResponseEntity<>(organisationService.findAllWhereMembershipIsNull(),
                    HttpStatus.OK);
        }
        if ("child-is-null".equals(filter)) {
            log.debug("REST request to get all Organisations where child is null");
            return new ResponseEntity<>(organisationService.findAllWhereChildIsNull(),
                    HttpStatus.OK);
        }
